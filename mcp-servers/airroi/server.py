...
Auth: AIRROI_API_KEY (header X-API-KEY). AirROI returns USD only.
...
def health_check() -> dict:
    ...
    estimate). If ok is false, set AIRROI_API_KEY in .env. AirROI returns USD only."""
...
def get_comparables(bedrooms: int, baths: float, guests: int,
                    ...
                    radius: int = 0, currency: str = "usd") -> dict:
    ...
    latitude+longitude. radius (miles) optionally widens thin markets. USD only."""
...
@mcp.tool()
def get_listing(listing_id: int, currency: str = "usd") -> dict:
    """Full detail for one Airbnb listing by AirROI listing_id: description, photos, host,
    location, specs, ratings, and performance metrics. USD only."""
    return ar.get_listing(listing_id, currency)


@mcp.tool()
def get_listing_metrics(listing_id: int, num_months: int = 12, currency: str = "usd") -> dict:
    """Monthly time-series for one listing: occupancy / ADR / revenue / RevPAR at the
    p25-p90 percentiles over num_months. USD only."""
