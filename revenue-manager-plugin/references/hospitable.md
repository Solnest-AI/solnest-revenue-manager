...
4. **Currency hard gate** — auto-detect the property's native currency (e.g. CAD) from `list_properties`/calendar. Never let a foreign-currency figure (e.g. AirROI USD) enter a recommendation without explicit conversion; on mismatch, convert or clearly flag.
